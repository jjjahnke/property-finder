...
def ingest_geodata():
    ...
    # --- Handle Duplicates ---
    if 'PARCELID' in gdf.columns:
        duplicates = gdf[gdf.duplicated(subset=['PARCELID'], keep=False)]
        if not duplicates.empty:
            logging.warning(f"Found {len(duplicates)} duplicated rows based on 'PARCELID'.")
            
            log_cols = ['STATEID', 'PARCELID', 'TAXPARCELID', 'PARCELDATE', 'TAXROLLYEAR', 'OWNERNME1']
            cols_to_log = [col for col in log_cols if col in duplicates.columns]
            
            duplicates_log_path = os.path.join(DATA_DIR, 'duplicates.csv')
            ...
            gdf.drop_duplicates(subset=['PARCELID'], keep='first', inplace=True)
            final_rows = len(gdf)
            logging.info(f"Removed {initial_rows - final_rows} duplicate records. {final_rows} unique records remain.")
        else:
            logging.info("No duplicate PARCELIDs found.")
